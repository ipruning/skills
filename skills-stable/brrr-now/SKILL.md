...
- 触发事件是什么。通知要挂在能观察到真实结果的位置上。会话内的等待用 harness 自己的等待机制，前台 `sleep` 只在普通 shell 脚本里可靠。要延后或反复触发的调度，一次性提醒和检查循环都交给 `$scheduled-work` 或 harness 自己的调度机制。只有目标主机上没有可用的调度器时，才自己落临时脚本、cron 或 systemd。
