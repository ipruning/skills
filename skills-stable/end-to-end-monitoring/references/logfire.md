...
If a dedicated Logfire skill is installed in the environment, it owns Logfire MCP tool usage and SQL dialect validation; this file owns the monitoring contract side: identity, tokens, and alert design.
...
monitor.id=<producer>/<subject-or-host>  # probes use <system>/<subject>/<check> instead
```
...
- Create one write token per revocation boundary. For one host, name it like `<project>/<service.name>/<host.name>` or `<project>/<monitor.id>`.
...
- Use separate tokens when monitors need different revocation boundaries; sharing one dedicated host token is acceptable only when the lifecycle and access boundary are the same.
- Record how to revoke the token.

When the user authorized token creation, an agent may read, copy, paste, and transiently pass the one-time token through the browser and SSH to finish the deployment. Protect the secrets boundary: do not commit the token, leave it in shell history, store it in unit files or config YAML, place it in ordinary user-writable files, or repeat it in the final report.
...
Prefer onset-only notification, such as `starts_having_matches`, when repeated notifications would create noise. That trigger name comes from Logfire's alert config; check the current Logfire alerts docs if it has drifted. Use change-based notification only when recovery messages are part of the contract.

Before claiming a channel verified, confirm the destination can consume Logfire's alert payload. A webhook URL alone does not prove delivery semantics. If Logfire sends a Slack-format payload to Feishu, Discord, enterprise chat, PagerDuty, or a custom webhook receiver, add an adapter or bridge and verify the translated notification end to end. Push to the user's phone can ride the brrr-now skill's sender when that skill is installed. In this contract, `responder` means the person or service that takes the response action, not the payload receiver.
...
7. The real responder or user confirms the notification was received and matches the intended interruption behavior.
