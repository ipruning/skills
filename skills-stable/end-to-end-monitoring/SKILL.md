...
hostmetrics monthly cost estimate: datapoints ~= hosts x 520M / interval_seconds (full scraper set ~200 datapoints per scrape); price it against the backend's rate before deploying, and state the estimate in the delivery report
hostmetrics freshness threshold: >=3x collection interval (60s -> 3m, 300s -> 15m); never below 3x or the alert flaps
custom probe freshness: start at 2x the probe cadence, cap at cadence + repair window; do not reuse the host default for hourly jobs
