...
UI 用 `packages/ui` 里的 shadcn + Base UI 组件，图标用 `lucide-react`。页面内容居中，有最大宽度。层级靠间距和字号表达。避免卡片套卡片、badge 堆砌。列表默认只读，编辑态点击才出现。Base UI 的 `Select` 触发器实际 role 是 `combobox`；测试定位不要把它当普通 button。`Dialog` 打开后应确认焦点落在第一个可编辑控件。移动端检查按钮和长标题是否完整可读。

只有当用户明确要 Cloudflare dashboard 风格或既有项目已经用 Kumo，才加 Kumo；Kumo 只进 web app，不进 Convex backend。
