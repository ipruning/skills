...
description: "Operate Linux servers over SSH: Debian/Ubuntu systemd setup and maintenance, distro-aware health and intrusion audits, SSH, firewall and container exposure, swap, and general server-side work for proxy/VPN services. Use for unclear or possibly compromised hosts as well as known hosts. Not for macOS network problems; all Snell-specific audit, deployment, repair, migration, and validation belong to operate-snell; REALITY+HY2 deployment, repair, validation, and its required host preflight belong entirely to sing-box-reality-hy2."
metadata:
  version: "8"
---
...
- 单主 VPS：用户直接管 root 的个人机或代理节点。保持 key-only root 的访问形状，只开当前需要或明确规划的端口。不默认加非 root 管理员、fail2ban、SSH 算法表或 sysctl 调优，机器没这个需要且用户没要求就不加。SSH 细节看 [references/ssh.md](references/ssh.md)，防火墙看 [references/firewall.md](references/firewall.md)。Snell 的 systemd、端口、防火墙和资源影响也归 `$operate-snell`；只有 Snell 之外的整机问题才回到这里。REALITY+HY2 归 `$sing-box-reality-hy2`。
