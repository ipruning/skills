...
神谕是一个比你更强的 AI，这样问它：`amp --mode ultra -x "$(cat /tmp/oracle.md)"`

它和你一样能读代码、跑命令、上网查资料，但它不知道咱们聊了什么，也没法反过来问你。所以把问题写进 /tmp/oracle.md 时要交代清楚：

- 你在做什么任务，目标是什么
- 你已经试过什么、排除了什么，别让它白查一遍
- 这次具体问它哪一件事，一次只问一件
- 代码在哪：仓库路径、分支、相关文件、怎么复现、怎么跑测试。让它自己去读，不用把代码贴给它
- 你的环境信息里有本线程的 Thread ID，把它也写给神谕，告诉神谕需要了解来龙去脉时可以用 read_thread 工具带着具体问题查这个线程，比如问用户最初的原话要求是什么
- 提醒它：委托书里你转述的进度和结论仅供参考，一切以代码现状、git 记录和线程原文为准，怀疑就自己去查
- 告诉它只调查、给意见，不要动手改代码
- 有含糊的地方，直接告诉它按什么假设来，因为它没法追问你

什么时候问：卡住两次以上的 bug、拿不定主意的设计选择、动手前想找人把关的计划、改完一大块想找人复查。小事别问，自己做更快。需要就多问几次，每次换个文件名。

这个查线程的办法你自己也能用：任务跑得很长、怀疑自己记忆被压缩过时，拿自己的 Thread ID 调 read_thread，问用户最初要求的原话是什么，别靠压缩后的印象猜。

它的答案要自己对着代码核实过再用，说错了就以实际情况为准。如果它说要解决必须改某个文件，先停下来向用户说明、拿到授权，再自己动手改。改代码始终是你的活，不是神谕的。

最后汇报时提一下问了什么、采纳了哪些、没采纳哪些。
